...
const GuardAttendanceTable = () => {
  ...
  const getDailyHours = (attendanceRecord, day) => {
    ...
    if (!attendanceRecord) {
      ...
      return {
        formattedHours: isSpecialDay ? "12:0" : "0:0",
        deductedLunch: "No",
      };
    }
    ...
    if (!checkInTime || !checkOutTime) {
      return {
        formattedHours:
          isHoliday(day) && !attendanceRecord?.removeDay ? "12:0" : "0:0",
        deductedLunch: "No",
      };
    }
    ...
    // Add 12 hours for Sundays or holidays
    if (isHoliday(day) && !attendanceRecord.removeDay && !isSunday(day)) {
      totalHours += 12;
    }
  ...
  const calculateOvertimeHours = (attendanceRecord) => {
    ...
    // Calculate overtime (subtracting 12 hours)
  ...
  const calculateTotalWorkingDays = (attendanceRecords) => {
    ...
    daysInMonth.forEach((day) => {
      ...
      if (
        ...
        (!attendanceRecord && isHoliday(day) && !isSunday(day)) || // If no record and it's a holiday
        (attendanceRecord &&
  ...
  const csvHeaders = [
    ...
    { label: "Total Overtime", key: "totalOvertime" },
    { label: "Total Hours", key: "totalHours" },
    { label: "Total Salary", key: "totalSalary" },
  ];

  const csvData = useMemo(() => {
    if (!currentEmployees) return []; // Ensure csvData is always an array
    ...
    return currentEmployees.map((employee, idx) => {
      const dailyHours = daysInMonth.map((day) => {
        ...
        if (!attendanceRecord) {
          return isHoliday(day) && !isSunday(day) ? 12 : 0;
        }
      ...
      return {
        serialNumber: idx + 1, // Serial number for each employee
        name: employee.name,
  ...
  const totalSalaryToBePaid = useMemo(() => {
    return currentEmployees?.reduce((acc, employee) => {
      ...
      return acc + (totalSalary || 0); // Ensure totalSalary is a number
    }, 0);
            ...
            {currentEmployees?.map((employee, idx) => (
              <Tr key={employee._id}>
                ...
                {daysInMonth.map((day) => {
                  ...
                  // const isCurrentDaySunday = isSunday(day);
                  const { formattedHours, shouldDeductLunch } = getDailyHours(
                    attendanceRecord,
                    day
                  );
                  const todaysSalary = calculateDailySalary(
                    employee.salary,
                    formattedHours,
                    daysInMonth.length
                  );
