...
const StaffAttendanceTable = () => {
  ...
  const extractTime = (isoString) => {
    if (!isoString) {
      return "00:00"; // Default time if isoString is undefined or null
    }

    const [date, time] = isoString.split("T");

    if (!time) {
      return "00:00"; // Default time if time part is missing
    }
  ...
  const csvData = useMemo(() => {
    if (!employees || !Array.isArray(employees)) {
      console.error("Employees data is not an array or is undefined");
      return [];
    }
    ...
    return employees.map((employee, index) => {
      const attendance = daysInMonth.reduce((acc, day) => {
        ...
        if (isCurrentDaySunday && !attendanceRecord) {
          ...
          status = getAttendance(attendanceRecord, day);
        } else {
          status = "A";
        }
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {getAttendance(attendanceRecord, day) === "Half day" ? (
                          <>
                            <RxCrossCircled
                              color="red"
